...
static const uint8_t tx_addr[3] = {0xb3, 0xb4, 0x01};
...
{
    CS_ON;
    HAL_SPI_Transmit(&hspi3, (uint8_t[]){ACTIVATE}, 1, 1000);
    Delay_us(NRF24_DELAY_US);
    HAL_SPI_Transmit(&hspi3, (uint8_t[]){0x73}, 1, 1000);
    CS_OFF;
}
...
{
    CS_ON;
    HAL_SPI_Transmit(&hspi3, (uint8_t[]){FLUSH_RX}, 1, 1000);
    CS_OFF;
}
...
{
    CS_ON;
    HAL_SPI_Transmit(&hspi3, (uint8_t[]){FLUSH_TX}, 1, 1000);
    CS_OFF;
}
